...
function TimeFilterSelector({ value, onChange, label, availableDates = [] }) {
  ...
  const [mode, setMode] = useState(typeof value === 'string' ? value : (value.mode || 'all'));
  ...
  useEffect(() => {
    ...
    const datesToProcess = availableDates && availableDates.length > 0
      ? availableDates
      : movements.map(m => m.timeIn || m.In || m.date).filter(Boolean);
    
    if (datesToProcess && datesToProcess.length > 0) {
      // Extract unique years
      const years = [...new Set(datesToProcess.map(date => new Date(date).getFullYear()))];
      years.sort((a, b) => a - b);
      setAvailableYears(years);
      
      // Extract unique months (in format YYYY-MM)
      const months = [...new Set(datesToProcess.map(date => {
      ...
      const quarters = [...new Set(datesToProcess.map(date => {
        const d = new Date(date);
      ...
      const days = [...new Set(datesToProcess.map(date => {
        const d = new Date(date);
